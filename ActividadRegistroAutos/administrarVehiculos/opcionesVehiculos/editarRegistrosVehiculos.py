...
class frameEditarRegistrosVehiculos(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.labelTitulo = tk.Label(self.contenedorTituloYDatos, text="Editar datos de un Vehiculo")
    ...
    def botonEditarFuncion(self):
        try:
            ...
            idEdicion = self.entryIdVehiculo.get()
            # Obtengo los Valores

            Marca = self.entryMarca.get()
            Modelo = self.entryModelo.get()
            Año = self.entryAño.get()
            Matricula = self.entryMatricula.get()
            NumeroSerie = self.entryNumeroSerie.get()

            # Hago los cambios
            Vehiculo.editarVehiculo(idEdicion,Marca,Modelo,Año,Matricula,NumeroSerie)
